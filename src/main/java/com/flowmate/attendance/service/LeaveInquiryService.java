 ...
 *
 * 계획서 4의 교차 모듈 규칙: approval 은 attendance 의 Service 인터페이스만 보고,
 * 매퍼(HolidayMapper·LeaveBalanceMapper)를 직접 호출하지 않는다. 기안 화면이
 * 필요로 하는 두 가지 — 영업일수 계산과 잔여 조회 — 를 이 인터페이스 하나로
 * 모은다.
 *
 * LeaveApplyService(Task 6, 승인 반영용 쓰기 경로)와는 목적이 다르다 — 이쪽은
 * 기안 시점의 읽기 전용 조회다.
 */
public interface LeaveInquiryService {
     ...
     * 승인 시점의 권위 있는 검증(잠금 안에서 재확인)을 대신하지 않는다 (계획서 D4).
     *
