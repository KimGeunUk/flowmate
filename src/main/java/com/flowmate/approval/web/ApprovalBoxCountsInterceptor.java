 ...
 *
 * ★ @ControllerAdvice 가 아니라 인터셉터인 이유: @ModelAttribute 를 단
 *   ControllerAdvice 는 @RestController 에도 걸려서 AI 요약·사전점검 같은 AJAX
 *   호출마다 집계 쿼리가 한 번씩 더 나간다. postHandle 은 뷰를 그리는 요청에만
 *   ModelAndView 를 주므로(@ResponseBody 는 null) 그 구분이 공짜로 된다.
 *
 * ★ 실패하면 조용히 넘어간다. 배지가 안 보일 뿐이고, 배지 하나 때문에 화면
 *   전체가 500 이 되면 안 된다 - 특히 오류 화면을 그리는 중에 여기서 또 터지면
 *   원래 오류가 무엇이었는지조차 알 수 없게 된다.
 */
@Component
