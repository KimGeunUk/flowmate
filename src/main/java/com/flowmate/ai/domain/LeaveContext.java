 ...
 *
 * LLM 을 쓰지 않는다 - ai 패키지에 있는 이유는 이 기능이 "AI 기능 3종" 중
 * 하나(결재와 근태의 통합을 보여주는 기능)로 묶여 있기 때문이지 LLM 호출이
 * 있어서가 아니다. 여기에 LLM 판단 코멘트를 더하는 안도 있었지만 범위에서
 * 가장 먼저 덜어냈다.
 *
 ...
 * 중복해서 나를 필요가 없다.
 *
 * ai 패키지가 approval/attendance 의 도메인 객체를 참조하는 것은 모듈 규칙상
