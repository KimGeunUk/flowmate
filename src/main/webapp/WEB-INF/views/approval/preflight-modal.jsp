...
<%--
  상신 전 사전 점검 모달 (설계서 §6.4.6, 계획서 5 Task 6).

  reject-modal.jsp 와 달리 이 모달의 내용(findings 목록)은 서버 렌더링이 아니라
  write.jsp 하단 스크립트가 상신 클릭 시점에 /api/ai/approvals/{id}/preflight 를
  불러 채운다 - 점검은 "지금 이 순간의 본문"에 대해 매번 새로 돌아야 하는데(D4,
  캐시하지 않는 이유와 같다), 화면 진입 시점에 이미 만들어 둘 수 없기 때문이다.
  ...
  근거함"이라는 숫자가 이 기능 전체의 존재 이유다(계획서 5 Task 5).
--%>
