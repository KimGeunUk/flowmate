-- AI 기능 스키마 (ai_preflight_result, ai_prompt).
--
-- ★ 이 파일은 두 경로에서 실행된다:
...
-- Phase 0~4 데이터를 전부 잃는 것이므로 금지한다. 반드시 두 번 실행해
...
COMMENT ON TABLE  ai_preflight_result             IS '사전점검 결과. 결재 문서 상신 직전 AI 점검 이력';
...
-- 커스터마이징 지점 4(PromptRepository)의 두 번째 구현(DatabasePromptRepository)이
...
COMMENT ON TABLE ai_prompt IS 'DatabasePromptRepository 가 읽는 프롬프트 저장소. File 구현과 같은 (feature, version) 키 규약을 쓴다';
