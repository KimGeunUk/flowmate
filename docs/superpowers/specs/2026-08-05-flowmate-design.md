...
| 5-5 | 기능 3b LLM 판단 코멘트 ← **버퍼 구간** | 1.0 | (밀리면 포기, 3a로 대체) |
...
| 단계 | 작업 | 분량 | 완료 기준 |
|---|---|---:|---|
| 6-1 | Docker 배포: WAR → Tomcat 10.1 컨테이너 + PostgreSQL. `docker-compose up`으로 기동 | 0.5 | 컨테이너에서 전체 기능이 동작한다 |
| 6-2 | README(아키텍처 다이어그램, 실행법, **설계 판단 기록**), `docs/oracle-mapping.md`, 커밋 이력 정리, 데모 시나리오 스크립트 | 0.5 | 처음 보는 사람이 README만 읽고 실행하고 의도를 이해할 수 있다 |
...
|---|---|---:|---:|---:|
| 0 | 환경 구축 | 1.5 | 1.5 | 1.5 |
| 1 | 조직 · 사용자 | 2.5 | 4.0 | 4.0 |
| 2 | 전자결재 코어 | 4.5 | 8.5 | 8.5 |
| 3 | AI 게이트웨이 | 1.0 | 9.5 | (2와 병행) |
| 4 | 근태 + 연동 | 3.0 | 12.5 | 11.5 |
| 5 | AI 기능 | 4.5 | 17.0 | 16.0 |
| 6 | 마감 | 1.0 | **18.0** | 17.0 |
| — | 예비 | — | **0** | **1.0** |

**직렬로 진행하면 예비일이 0이다.** Phase 3을 Phase 2와 병행해야 1일의 완충이 생긴다.
Phase 2에서 막힐 때 Phase 3으로 전환하는 것이 단순한 기분 전환이 아니라 **일정상 필요한 조치**임을 뜻한다.
