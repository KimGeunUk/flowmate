...
FlowMate 는 PostgreSQL 16 으로 개발한다. 그룹웨어는 회사마다 쓰는 DB 가 다르고 Oracle 인 곳도
많으므로, **PostgreSQL 전용 문법을 쓸 때마다 그 자리에서 Oracle 대응을 한 줄 적어 둔다.**
사후에 몰아 쓴 문서가 아니라는 것이 이 문서의 전부다 - 나중에 옮길 일이 생겼을 때
어디를 봐야 하는지가 이미 적혀 있다.
